    ...
    for (const ats of Object.values(cl)) {
      ...
      for (const entry of ats) {
        if (entry.default_domain) {
          // Slug must match the fetcher's company_slug output exactly.
          // GH/Lever/Ashby/SR: entry.slug is already the canonical slug.
          // WD: no slug field — must match WD fetcher's tenantName.toLowerCase().replace(/\s+/g, '-')
          let slug;
          if (entry.slug) {
...
function tagDomains(job, options) {
  ...
  // L6 respects TAG-GUARD-4: if L1 blocked software for this dept (e.g. bare "IT"), L6 must
  // not re-add it via family map. The guard exists because these depts are operations, not SWE.
  const l6Guarded = deptGuardedSoftware;
  if (tags.length === 0 && !l6Guarded && deptRaw && WD_FAMILY_MAP.has(deptRaw)) {
    const familyDomain = WD_FAMILY_MAP.get(deptRaw);
    if (familyDomain !== 'general') {
