...
// Added 2026-06-21 (AGG-NEWGRAD-SUPFILTER-1): branch manager (bank/retail branch leads) +
// site manager (construction/ops/data-center site leads). Live-measured vs R2: 121 branch +
// 9 site US rows leaking past the filter; 0 FPs (assistant/trainee/rotational variants
// preserved by guards). "site" uses \b so it never matches the "site" in "website/onsite".
...
// "operations supervisor" INCLUDED 2026-06-19 after per-job research: all 34 US rows are
// line leads (Lowe's/CVS retail ops, ASM Global/Live Nation venue ops, Eversource grid-ops,
// SpaceX/Danaher/Thermo Fisher mfg ops, Veolia wastewater, lab ops) — none new-grad.
...
// Added 2026-06-21 (AGG-NEWGRAD-SUPFILTER-1): shift/field service/retail/store/floor/center
// supervisor (retail + skilled-trade line leads). Live-measured vs R2: 30 shift + 39 retail/
// store/floor/center + 2 field-service US rows leaking; 0 FPs. "shift" also catches
// "...Supervisor (Night/2nd/3rd Shift)" variants — all line supervisors, correctly filtered.
// Added 2026-06-21 (AGG-NEWGRAD-SUPFILTER-2): industrial/supply-chain/technical line-supervisor
// niches — logistics/materials/shipping (supply chain), assembly/molding (manufacturing),
// lab/metrology/radiological (technical-scientific leads), ramp/cargo (airport ops),
// switchgear/neta/iron/meter (electrical/utility field), mechanic, ehs. Live-measured vs R2:
// 31 US rows newly caught, 0 FPs (all skilled-trade/ops line leads; guards preserve
// assistant/associate/trainee/rotational variants). DEFERRED to separate per-category decisions
// (different domain or broad future-FP risk): training (rotational-program FP risk), security,
// culinary/food/restaurant (hospitality), nurse/clinical/rn/phlebotomy (healthcare), office/
// corporate supervisors (finance/compliance/client-services/project).
// Added 2026-06-21 (AGG-NEWGRAD-SUPFILTER-3): final consolidation of remaining clearly-not-new-grad
// line-supervisor domains — healthcare clinical (nurse/clinical/phlebotomy), venue hospitality
// (culinary/food/restaurant/beverage), venue/event security. Live-measured vs R2: 24 US rows newly
// caught, 0 FPs (all experienced clinical / venue food-service / event-security line supervisors).
// This closes the clearly-safe supervisor domains. Added 2026-06-21 (AGG-NEWGRAD-SUPFILTER-4):
// `training` — technical/industrial training supervisors (weld/equipment/ops trainers). Live-measured
// vs R2: 5 US rows, 0 FPs, NONE near program/rotational/development/graduate (the rotational-FP risk
// does not manifest; ENTRY_LEVEL_GUARDS + ROTATIONAL_GUARD still preserve those variants if they appear).
// Added 2026-06-25 (AGG-NEWGRAD-SUPFILTER-5): office/corporate supervisors — the previously DEFERRED
// office/corporate bucket, now resolved by per-category data (agent://SUP-NEWGRAD-QUANT). NONE of the
// 34 leaked US office/corporate supervisor titles are new-grad-appropriate — all are experienced
// people-management leads: finance/accounting/revenue-cycle/AR/billing, customer-service/client-
// services/community/complaint, IT-service/service-delivery/application-support/dispatch/technical-
// service, risk/AML/compliance/tariff, HR/benefits/administrative/payroll/office, audit, and corporate
// project/e-discovery. Live-measured vs R2: 34 US rows leaked past the filter (31 noun-form + 3
// adjective-form "Supervisory X"); this change filters 33 of them, 0 FPs. The 1 residual —
// "Supervisory Clerical (Executive Assistant)" @ CACI — is exempted by the entry-level "Assistant"
// guard (which must be preserved); it can't be noun-matched either ("Supervisory" is an adjective,
// not the noun "supervisor"), so it is a known guard-bound residual, not a regex gap.
// assistant/associate/junior/trainee/rotational variants are preserved by ENTRY_LEVEL_GUARDS +
// ROTATIONAL_GUARD. Tokens are a subset of the quant's measured-0-FP set. The noun-form regex matches
// "supervisor" (noun) in either word order within 30 chars; the separate
// `\bsupervisory\s+(accountant|accounting|clerk|clerical)\b` clause catches the adjective form.
// NOTE on `audit`: there is no \bintern\b guard here and every ENTRY_LEVEL_GUARD uses word boundaries,
// so "Supervisor, Internal Audit" is filtered cleanly (no false exemption on "Internal"). Still
// deferred: compound `site X manager` variants.
