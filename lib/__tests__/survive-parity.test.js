...
test('Ashby GraphQL normalization preserves team hierarchy and stable URL', () => {
  ...
  // AGG-SALARY-EXTRACT-1: flat annual salary fields from structured compensation.
  assert.strictEqual(job.salaryMin, 10);
  assert.strictEqual(job.salaryMax, 20);
  assert.strictEqual(job.salaryCurrency, 'USD');
});
...
test('Ashby GraphQL detail normalization preserves published date and description', () => {
  ...
  // AGG-SALARY-EXTRACT-1: flat fields parsed from the '$200K - $260K' string.
  assert.strictEqual(job.salaryMin, 200000);
  assert.strictEqual(job.salaryMax, 260000);
});

test('Ashby location parser handles Canadian province names (AGG-CANADA-FETCHZERO-1 / NONUS Tier 2)', () => {
  // Regression: CANADA_PROVINCE_NAME_TO_ABBR was used but not imported → ReferenceError crashed 51 companies.
  // Tested via normalizeAshbyJob (posting-api path) — the path that calls parseAshbyLocation.
  const job = normalizeAshbyJob({
    id: 'prov-on', title: 'Eng', location: 'Toronto, Ontario',
    employmentType: 'Full-time', publishedAt: '2026-07-01',
  }, 'testco', 'TestCo');
  assert.strictEqual(job.job_state, 'ON');
  assert.strictEqual(job.job_city, 'Toronto');

  const jobBC = normalizeAshbyJob({
    id: 'prov-bc', title: 'Eng', location: 'Vancouver, British Columbia',
    employmentType: 'Full-time', publishedAt: '2026-07-01',
  }, 'testco', 'TestCo');
  assert.strictEqual(jobBC.job_state, 'BC');
});

test('Ashby location parser handles US state names (AGG-LOC)', () => {
  const job = normalizeAshbyJob({
    id: 'state-ca', title: 'Eng', location: 'San Francisco, California',
    employmentType: 'Full-time', publishedAt: '2026-07-01',
  }, 'testco', 'TestCo');
  assert.strictEqual(job.job_state, 'CA');
  assert.strictEqual(job.job_city, 'San Francisco');

  // 2-letter codes still work (the path that never crashed)
  const job2 = normalizeAshbyJob({
    id: 'state-wa', title: 'Eng', location: 'Seattle, WA',
    employmentType: 'Full-time', publishedAt: '2026-07-01',
  }, 'testco', 'TestCo');
  assert.strictEqual(job2.job_state, 'WA');
});

test('Simplify target set: AGG-SIMPLIFY-EXIT-1 removed companies with direct fetchers; remaining targets preserved', () => {
  // TikTok (and 13 others) removed — they have direct fetchers now (TikTok: 118 simplify + 1030 direct).
  assert(!TARGET_COMPANIES.includes('TikTok'), 'TikTok should be removed (has direct fetcher)');
  // ByteDance, Goldman Sachs, Shopify still present — no direct fetcher yet.
  ...
  assert.strictEqual(isTargetCompanyName('TikTok', targetSet), false);
