...
(async () => {
  ...
  const schemaDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sr-desc-schema-'));
  fs.writeFileSync(path.join(schemaDir, 'descriptions-smartrecruiters.jsonl'), JSON.stringify({
    id: 'sr-testco-4000',
    description_text: 'Old description only',
  }) + '\n');
  const schemaCalls = [];
  const schemaRetried = await fetchSRDescriptions([{ id: 'sr-testco-4000', company_slug: 'testco', source_id: '4000' }], schemaDir, {
    maxPerRun: 1,
    concurrency: 1,
    delayMs: 0,
    fetchJson: async (url) => {
      ...
      return {
        jobAd: { sections: {
          jobDescription: { text: '<p>Responsibilities here</p>' },
          qualifications: { text: '<p>Bachelor of Science in Engineering</p>' },
        } }
      };
    }
  });
  assert.strictEqual(schemaCalls.length, 1, 'legacy-schema sidecars should refetch once');
  assert.ok(schemaRetried.get('sr-testco-4000').includes('Bachelor of Science in Engineering'));
  fs.rmSync(schemaDir, { recursive: true, force: true });
  const priorityDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sr-desc-priority-'));
  const priorityJobs = [
    { id: 'sr-testco-5000', company_slug: 'testco', source_id: '5000', title: 'Senior Platform Manager', employment_type: 'full-time' },
    { id: 'sr-testco-5001', company_slug: 'testco', source_id: '5001', title: 'Mechanical Engineering Intern', employment_type: 'internship' },
  ];
  const priorityCalls = [];
  const prioritized = await fetchSRDescriptions(priorityJobs, priorityDir, {
    maxPerRun: 1,
    concurrency: 1,
    delayMs: 0,
    fetchJson: async (url) => {
      ...
      return {
        jobAd: { sections: {
          jobDescription: { text: '<p>Internship work</p>' },
          qualifications: { text: '<p>Bachelor of Science in Engineering</p>' },
        } }
      };
    }
  ...
  assert.ok(priorityCalls[0].endsWith('/5001'), 'internship stale sidecars should be prioritized into bounded refresh batch');
  assert.ok(prioritized.get('sr-testco-5001').includes('Bachelor of Science in Engineering'));
  fs.rmSync(priorityDir, { recursive: true, force: true });
