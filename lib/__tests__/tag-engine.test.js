...
test('flightline operations manager → sales', () => {
  const domains = tagDomains(makeJob('Flightline Operations Manager'));
  assert.ok(domains.includes('sales'), 'flightline operations manager is sales');
});
...
test('field technical manager → manufacturing', () => {
  const domains = tagDomains(makeJob('Field Technical Manager (Lansing, MI)'));
  assert.ok(domains.includes('manufacturing'), 'field technical manager is manufacturing');
});
