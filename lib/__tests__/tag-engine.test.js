...
test('JPM investment banking title → finance (company domain override)', () => {
  const domains = tagDomains(makeJob('Investment Banking [Multiple Positions Available]', { company_name: 'JPMorgan Chase' }));
  assert.ok(domains.includes('finance'), 'JPM investment banking title should use company finance override');
});

test('JPM commercial term lending client specialist → finance (company domain override)', () => {
  const domains = tagDomains(makeJob('Client Specialist - Commercial Term Lending, Commercial Real Estate', { company_name: 'JPMorgan Chase' }));
  assert.ok(domains.includes('finance'), 'JPM commercial term lending client specialist should use company finance override');
});

test('JPM securities based lending analyst → finance (company domain override)', () => {
  const domains = tagDomains(makeJob('JPMorgan Wealth Management Securities Based Lending Analyst', { company_name: 'JPMorgan Chase' }));
  assert.ok(domains.includes('finance'), 'JPM securities based lending analyst should use company finance override');
});

test('JPM finance & business management associate → finance (company domain override)', () => {
  const domains = tagDomains(makeJob('Finance & Business Management - Associate', { company_name: 'JPMorgan Chase' }));
  assert.ok(domains.includes('finance'), 'JPM finance & business management associate should use company finance override');
});

test('JPM finance and business management → finance (company domain override)', () => {
  const domains = tagDomains(makeJob('Finance and Business Management', { company_name: 'JPMorgan Chase' }));
  assert.ok(domains.includes('finance'), 'JPM finance and business management should use company finance override');
});
