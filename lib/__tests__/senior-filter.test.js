...
(function newGradSupFilter2() {
  ...
  const shouldKeep = [
    ...
    'Software Engineer',
    // DEFERRED-domain supervisor still not filtered at SUPFILTER-2 time (broad future-FP risk).
    'Training Supervisor',                    // still deferred (rotational-program FP risk)
    // (security/culinary/nurse-clinical were deferred here but moved to FILTERED in AGG-NEWGRAD-SUPFILTER-3)
  ];
  for (const title of shouldKeep) {
    assert.ok(!hasSeniorTitle(title), `expected to KEEP (not filter): "${title}"`);
  }
...
(function newGradSupFilter3() {
  ...
  const shouldKeep = [
    ...
    'Training Supervisor',                      // deferred (rotational-FP risk)
