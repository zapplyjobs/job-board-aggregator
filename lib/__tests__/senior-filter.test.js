...
(function newGradSupFilter2() {
  ...
  const shouldKeep = [
    ...
    'Software Engineer',
    // DEFERRED-domain supervisors are intentionally NOT yet filtered (no regression).
    'Training Supervisor',                    // deferred (broad / rotational-FP risk)
    'Security Supervisor',                    // deferred (physical-security domain)
    'Culinary Supervisor',                    // deferred (hospitality)
    'Nurse Clinical Supervisor',              // deferred (healthcare)
  ];
  for (const title of shouldKeep) {
    assert.ok(!hasSeniorTitle(title), `expected to KEEP (not filter): "${title}"`);
  }
