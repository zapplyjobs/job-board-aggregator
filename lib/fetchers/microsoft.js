...
const { abbreviateCanadianProvince } = require('./country-resolver');
...
function parseLocation(locations, standardizedLocations) {
  ...
  return {
    ...
    job_city: city || null,
    job_state: isUS ? (state || null) : isCA ? (abbreviateCanadianProvince(state) || null) : null,
  };
}
...
function isRetryableSearchStatus(status) {
  return status === 429 || status === 408 || status === 502 || status === 503 || status === 504 || status === 522 || status === 524;
}

async function fetchSearchPage(url, pageNumber, { getJsonImpl = getJson, delayImpl = delay } = {}) {
  for (let attempt = 0; attempt <= SEARCH_MAX_RETRIES; attempt++) {
    const result = await getJsonImpl(url);
    if (result && result.status === 200 && result.data?.data?.positions) {
      return result;
    }

    const status = result?.status || 'null';
    if (!isRetryableSearchStatus(status) || attempt === SEARCH_MAX_RETRIES) {
      console.log(`  Search page ${pageNumber}: status=${status}, stopping`);
      return result;
    }

    const backoff = SEARCH_RETRY_DELAY_MS * Math.pow(2, attempt);
    console.log(`  Search page ${pageNumber}: HTTP ${status}, retry ${attempt + 1}/${SEARCH_MAX_RETRIES} in ${backoff}ms`);
...
async function fetchSearchPages(maxPages, location = null, options = {}) {
  const { delayImpl = delay } = options;
  const positions = [];
  let start = 0;
  let pages = 0;

  while (pages < maxPages) {
        const url = `${BASE_URL}${SEARCH_PATH}?domain=${DOMAIN}&start=${start}&sort_by=post_date${location ? '&location=' + encodeURIComponent(location) : ''}`;
    const result = await fetchSearchPage(url, pages + 1, options);
    ...
    start += PAGE_SIZE;
    if (pages < maxPages) await delayImpl(SEARCH_DELAY_MS);
  }
...
function getMicrosoftFetchPlan(previousJobCount = 0, fetchDetailsOnInitial = false) {
  const isInitial = previousJobCount === 0;
  return {
    isInitial,
    maxPages: isInitial ? Infinity : MAX_ROUTINE_PAGES,
    skipDetails: isInitial && !fetchDetailsOnInitial,
  };
}

async function fetchAllMicrosoftJobs({ previousJobCount = 0, cachedDescriptionIds, fetchDetailsOnInitial = false } = {}) {
  console.log('\n🖥️  Fetching from Microsoft PCSX...');
  console.log('━'.repeat(60));

  const { isInitial, maxPages, skipDetails } = getMicrosoftFetchPlan(previousJobCount, fetchDetailsOnInitial);
  ...
  // Canada-specific search (SUP-MSCANADA-1: extend to Canada)
  console.log('  Phase 1b: Canada search (PCSX location=Canada)');
  const caPositions = await fetchSearchPages(10, 'Canada');
  const globalIds = new Set(positions.map(p => p.id));
  const newCa = caPositions.filter(p => !globalIds.has(p.id));
  positions.push(...newCa);
  console.log(`  Canada: ${caPositions.length} found, ${newCa.length} new (after dedup vs global)`);
...
module.exports = { fetchAllMicrosoftJobs, normalizePosition, stripHtml, parseLocation, fetchSearchPages, fetchSearchPage, isRetryableSearchStatus, getMicrosoftFetchPlan };
