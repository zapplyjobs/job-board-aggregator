...
function normalizeUberJob(job) {
  ...
  const applyUrl = jobId ? `https://www.uber.com/global/en/careers/list/${jobId}/` : null;
...
async function fetchAllUberJobs() {
  ...
  while (page <= MAX_PAGES) {
    ...
    const result = await postJson(API_URL, payload, { headers: HEADERS });
    ...
    const uberCountry = resolveCountry("uber", "US");
  const usJobs = results.filter(j => j.location?.countryName === uberCountry);
