...
const DELAY_MS = 300;
const ASHBY_GRAPHQL_URL = 'https://jobs.ashbyhq.com/api/non-user-graphql?op=ApiJobBoardWithTeams';
const ASHBY_GRAPHQL_QUERY = `query ApiJobBoardWithTeams($organizationHostedJobsPageName: String!) {
...
function buildAshbyLookupGraphQL(slug) {
  return {
...
async function probeAshby(slug) {
  ...
  const result = await getJson(url, { timeout: 10000 });
  if (result?.status === 200 && result.data?.jobs?.length) {
  ...
  const graphResult = await postJson(ASHBY_GRAPHQL_URL, buildAshbyLookupGraphQL(slug), {
    timeout: 10000,
    headers: {
  ...
  const jobs = graphResult?.data?.data?.jobBoard?.jobPostings || [];
  if (!jobs.length) {
    return null;
  }
  return { platform: 'ashby', slug, jobCount: jobs.length };
}
