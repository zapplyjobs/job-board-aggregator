...
async function fetchGreenhouseJobs(companySlug, companyName) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        ...
        if (!result) {
            ...
            console.log(`   ⚠️ Greenhouse network error for ${companySlug} after ${MAX_RETRIES + 1} attempts`);
            return [];
        }
        ...
        if (result.status === 200 && result.data) {
            ...
            return jobs.map(job => normalizeGreenhouseJob(job, companySlug, companyName));
        }
        ...
        // Non-retryable error
        console.log(`   ⚠️ Greenhouse API error for ${companySlug}: ${result.status}`);
        return [];
    }
    return [];
}
...
function parseGreenhouseLocation(locationStr) {
    ...
    if (parts.length >= 2) {
        ...
        // AGG-NONUS-REFACTOR-1 Tier 2: Canadian province name recognition
        const caAbbrFromSecond = CANADA_PROVINCE_NAME_TO_ABBR[parts[1].toLowerCase()];
        if (caAbbrFromSecond) {
            return { job_city: parts[0], job_state: caAbbrFromSecond };
        }
    }

    // Single part — check if it's a full US state name or Canadian province name
    if (parts.length === 1) {
        const abbr = STATE_NAME_TO_ABBR[parts[0].toLowerCase()];
        if (abbr) {
            return { job_city: '', job_state: abbr };
        }
        const caAbbr = CANADA_PROVINCE_NAME_TO_ABBR[parts[0].toLowerCase()];
        if (caAbbr) {
            return { job_city: '', job_state: caAbbr };
        }
    }
...
function normalizeGreenhouseJob(job, companySlug, companyName) {
    ...
    return {
        ...
        // Dates — preserve first_published for display/audit, plus source_updated_at
        // for current-source active-window decisions on long-lived internships.
