...
let _rateLimitedCount = 0;  // AGG-WD-429MONITOR-1: tracks HTTP 429 responses per run
...
function postJson(url, body) {
    return new Promise((resolve) => {
        ...
        const req = https.request(url, options, (res) => {
            ...
            res.on('end', () => {
                if (res.statusCode === 429) _rateLimitedCount++;  // AGG-WD-429MONITOR-1
                try {
...
async function fetchAllWorkdayJobs(tenants, options = {}) {
    ...
    _rateLimitedCount = 0;  // AGG-WD-429MONITOR-1: reset per run
    const currentTotals = {};
    ...
    console.log(`   ⚠️  WD rate-limited (429): ${_rateLimitedCount} responses`);
    return { jobs: allJobs, currentTotals, segmentCache, rateLimited: _rateLimitedCount };
}
