...
async function fetchWorkdayJobs(tenant) {
    ...
    // Tenant-specific query windows recover important rows hidden behind Workday's generic
    // first-500 cap without changing global Workday behavior. Keep this opt-in and bounded.
        ...
        for (const searchText of tenant.searchQueries) {
            ...
            while (queryFetched < queryCap) {
                ...
                if (!result || result.status !== 200 || !result.data) {
                    console.log(`   ⚠️ Workday query error ${result ? result.status : 'network'}: ${name} searchText=${searchText}`);
                    break;
                }
...
async function fetchAllWorkdayJobs(tenants, options = {}) {
    ...
    if (hasCache) {
        ...
        // AGG-STALEUPSTREAM-1 (2026-07-04): rotate by OLDEST last_full_fetch_at first, not name-bucket.
        // The name-bucket rotate had coverage holes — some unchanged tenants were missed for days, so their
        // closed jobs lingered in the pool as silent residue (measured: 4.6d p50). Oldest-first GUARANTEES
        // bounded freshness: every tenant is full-fetched within ~(tenants/batchSize) cycles (~3h), so a
        // closed job is confirmed + dropped within hours. last_full_fetch_at is updated on each full fetch
        // (Phase 2), so this is self-correcting (a just-fetched tenant sorts last).
        // INVARIANT (OPERATOR 2026-07-04): this only reorders UNCHANGED tenants — changed-count tenants
        // (new jobs) are fetched first regardless. Cleaning never competes with fresh-job capture.
        refreshableUnchanged.sort((a, b) => (a._lastFullFetchAt || 0) - (b._lastFullFetchAt || 0));
