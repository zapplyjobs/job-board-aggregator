...
async function applyFamilyCache(jobs, tenants, cacheDir) {
    ...
    if (!cache || !cache.tenants) {
        console.log('   📦 No valid family cache — falling back to live mapping');
        const result = await annotateFamilyDepartments(jobs, tenants);
...
async function buildFamilyCache(tenants, cacheDir) {
    ...
    return { tenants: tenantCount, annotated: totalAnnotated, durationMs };
}
