 ...
 * @param {boolean} [options.skipFamilyMapping] - Skip family annotation (default: false)
...
async function fetchAllWorkdayJobs(tenants, options = {}) {
    const { concurrency = 15, delayMs = 200, previousTotals = null, skipFamilyMapping = false } = options;
    const allJobs = [];
    const currentTotals = {};
    ...
    // Phase 3: Annotate jobs with jobFamilyGroup via facet mapping (AGG-WD-DEPT-1)
    // For each tenant that produced jobs, fetch family facets then paginate per family
    // to build externalPath → familyName mapping. Annotates departments field.
    if (!skipFamilyMapping && allJobs.length > 0) {
        ...
        const famConcurrency = Math.min(concurrency, 5); // lower concurrency for family queries
        ...
        for (let i = 0; i < tenantNames.length; i += famConcurrency) {
            ...
            const results = await Promise.all(batch.map(async name => {
                ...
                try {
                    ...
                    return 0; // non-fatal — jobs keep departments: []
                }
            }));
        ...
        console.log(`   🏷️ Family mapping: ${annotated}/${allJobs.length} jobs in ${(familyMs / 1000).toFixed(1)}s`);
    }
