...
const UNCHANGED_REFRESH_BATCH_SIZE = 10;
...
async function probeWorkdayTotal(tenant) {
    const wdUrl = tenant.url.startsWith('http') ? tenant.url : `https://${tenant.url}`;
    const hostname = new URL(wdUrl).hostname;
    const tenantSlug = tenant.tenant || (hostname.includes('myworkdaysite.com') ? null : hostname.split('.')[0]);
    if (!tenantSlug) return null;
    const endpoint = `${wdUrl}/wday/cxs/${tenantSlug}/${tenant.site}/jobs`;
...
/**
 * Fetch jobFamilyGroup facet values for a WD tenant.
 * Returns facet array with {id, descriptor, count} per family, or null on failure.
 * AGG-WD-DEPT-1
 */
 ...
 * AGG-WD-DEPT-1
 */
async function buildFamilyPathMap(endpoint, families) {
    const pathMap = new Map();
    if (!families) return pathMap;
    for (const fam of families) {
        if (!fam.count) continue;
        let offset = 0;
        while (offset < fam.count) {
...
async function fetchAllWorkdayJobs(tenants, options = {}) {
    ...
    if (hasCache) {
        ...
        // Compare with cache — fetch tenants where total changed, probe failed, or the last full fetch is stale.
        const staleRefreshCandidates = [];
        changedTenants = [];
        ...
        for (const t of tenants) {
            ...
            currentTotals[t.name] = { total: curr, last_full_fetch_at: prevFullFetchAt || nowIso };
            if (prevTotal === undefined || prevTotal !== curr) {
                changedTenants.push(t);
                continue;
            }
            const staleMs = prevFullFetchAt ? nowMs - new Date(prevFullFetchAt).getTime() : Number.POSITIVE_INFINITY;
            if (!Number.isFinite(staleMs) || staleMs >= UNCHANGED_REFRESH_INTERVAL_MS) {
                staleRefreshCandidates.push({ tenant: t, staleMs });
                continue;
            }
            skippedCount++;
        }

        staleRefreshCandidates.sort((a, b) => b.staleMs - a.staleMs || a.tenant.name.localeCompare(b.tenant.name));
        const staleRefreshTenants = staleRefreshCandidates.slice(0, UNCHANGED_REFRESH_BATCH_SIZE).map(x => x.tenant);
        for (const t of staleRefreshTenants) changedTenants.push(t);
        skippedCount += Math.max(0, staleRefreshCandidates.length - staleRefreshTenants.length);

        console.log(`   ⏩ Skipped ${skippedCount} unchanged tenants, refreshing ${staleRefreshTenants.length} stale unchanged tenants, fetching ${changedTenants.length}`);
    }
        ...
        const results = await Promise.all(batch.map(async name => {
            ...
            try {
                ...
                for (const job of tenantJobs) {
                    const rawPath = job._raw?.externalPath;
 ...
 */
async function buildFamilyCache(tenants, cacheDir) {
    const start = Date.now();
    const MAX_DURATION_MS = 15 * 60 * 1000; // 15 min time budget — partial cache is better than timeout
    const cache = {
    ...
    for (let i = 0; i < tenants.length; i += famConcurrency) {
        ...
        const batch = tenants.slice(i, i + famConcurrency);
        const results = await Promise.all(batch.map(async tenant => {
            const wdUrl = tenant.url.startsWith('http') ? tenant.url : `https://${tenant.url}`;
            const hostname = new URL(wdUrl).hostname;
            const tenantSlug = tenant.tenant || (hostname.includes('myworkdaysite.com') ? null : hostname.split('.')[0]);
            ...
            try {
                ...
                const pathMapObj = await buildFamilyPathMap(endpoint, families);
