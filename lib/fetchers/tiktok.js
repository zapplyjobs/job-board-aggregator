...
async function fetchAllTiktokJobs() {
  ...
  // AGG-GIANTCANADA-1: Canada location code research round 2.
  // Round 1: CT|7 and CT_7 with recruitment_type_id_list:['2'] → 0 jobs.
  // Round 2 hypotheses:
  //   H1: Canada jobs use a DIFFERENT recruitment type (not campus type '2')
  //   H2: Empty location_code_list returns ALL global jobs (post-filter for Canada)
  for (const [label, probeBody] of [
    ['CT|7 + all-types', { location_code_list: ['CT|7'], recruitment_type_id_list: [], limit: 1, offset: 0 }],
    ['CT|7 + type-1',    { location_code_list: ['CT|7'], recruitment_type_id_list: ['1'], limit: 1, offset: 0 }],
    ['empty-loc + type-2', { location_code_list: [], recruitment_type_id_list: ['2'], limit: 1, offset: 0 }],
  ]) {
    try {
      const probeResult = await postJson(API_URL, probeBody, { headers: HEADERS, timeout: 10000 });
      if (probeResult?.data?.code === 0 && probeResult.data.data) {
        const count = probeResult.data.data.count || 0;
        const firstCity = probeResult.data.data.job_post_list?.[0]?.city_info?.en_name || '?';
        const firstCountry = probeResult.data.data.job_post_list?.[0]?.city_info?.parent?.parent?.en_name || '?';
        console.log(`  🇨🇦 TikTok probe (${label}): ${count} jobs — first: ${firstCity}, ${firstCountry}`);
      } else {
        console.log(`  🇨🇦 TikTok probe (${label}): API code=${probeResult?.data?.code ?? 'null'}`);
      }
    } catch (e) {
      console.log(`  🇨🇦 TikTok probe (${label}): error — ${e.message}`);
    }
    await delay(200);
  }
