...
async function fetchAllTiktokJobs() {
  ...
  // AGG-GIANTCANADA-1: Probe Canada location codes (isolated — doesn't affect US fetch).
  // US codes use CT_ prefix (underscore). Search page uses CT|7 (pipe). Testing both.
  for (const caCode of ['CT|7', 'CT_7']) {
    try {
      const probeResult = await postJson(API_URL, {
        location_code_list: [caCode],
      ...
      if (probeResult?.data?.code === 0 && probeResult.data.data) {
        ...
        console.log(`  🇨🇦 TikTok Canada probe (${caCode}): ${count} jobs — format ${count > 0 ? 'WORKS' : 'accepted but empty'}`);
      } else {
        console.log(`  🇨🇦 TikTok Canada probe (${caCode}): API code=${probeResult?.data?.code ?? 'null'} — rejected`);
      }
    } catch (e) {
      console.log(`  🇨🇦 TikTok Canada probe (${caCode}): error — ${e.message}`);
    }
    await delay(200);
  }
