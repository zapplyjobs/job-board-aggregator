 ...
 *
 * SUP-FETCHER-15: Custom fetcher for TikTok - replaces SimplifyJs single-point-of-failure.
 * TikTok/ByteDance removed from SimplifyJs TARGET_COMPANIES after fetcher ships.
 */
...
async function fetchAllTiktokJobs() {
  ...
  while (allJobs.length < MAX_JOBS) {
    ...
    if (apiData.code !== 0) {
      console.log(`  ⚠️ TikTok: API error code=${apiData.code}, stopping`);
      break;
    }
