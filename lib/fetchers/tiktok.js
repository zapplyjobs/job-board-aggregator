...
async function fetchAllTiktokJobs() {
  ...
  // AGG-GIANTCANADA-1: Post-filter probe — same code structure as round 1 (which worked).
  for (const label of ['empty-loc']) {
    try {
      ...
      if (probeResult?.data?.code === 0 && probeResult.data.data) {
        ...
        console.log(`  🇨🇦 TikTok probe (${label}): ${count} jobs — ${count > 0 && count <= 3000 ? 'POST-FILTER VIABLE' : count > 3000 ? 'TOO MANY' : 'empty'}`);
      } else {
        console.log(`  🇨🇦 TikTok probe (${label}): API code=${probeResult?.data?.code ?? 'null'} — rejected`);
      }
    } catch (e) {
