...
async function fetchAllTiktokJobs() {
  ...
  // AGG-GIANTCANADA-1: Post-filter approach test (same as ByteDance).
  // Round 1 proved CT|7 and CT_7 return 0 campus-type Canada jobs.
  // This probe tests: does empty location_code_list return ALL global campus jobs?
  // If yes + count < MAX_JOBS, we can post-filter for Canada.
  // Using console.error to bypass any stdout buffering issues from prior rounds.
  try {
    const globalProbe = await postJson(API_URL, {
      location_code_list: [],
      recruitment_type_id_list: ['2'],
      limit: 1,
      offset: 0,
    }, { headers: HEADERS, timeout: 10000 });
    if (globalProbe?.data?.code === 0 && globalProbe.data.data) {
      const gc = globalProbe.data.data.count || 0;
      const fj = globalProbe.data.data.job_post_list?.[0];
      const loc = fj ? `${fj.city_info?.en_name || '?'}, ${fj.city_info?.parent?.parent?.en_name || '?'}` : 'none';
      process.stderr.write(`[TIKTOK-PROBE] empty location_code_list: ${gc} global campus jobs (first: ${loc})\n`);
      if (gc > 0 && gc <= 3000) process.stderr.write(`[TIKTOK-PROBE] Post-filter VIABLE — ${gc} jobs fit in MAX_JOBS\n`);
      else if (gc > 3000) process.stderr.write(`[TIKTOK-PROBE] Post-filter NOT viable — ${gc} > MAX_JOBS\n`);
    } else {
      process.stderr.write(`[TIKTOK-PROBE] empty location_code_list: API rejected (code=${globalProbe?.data?.code ?? 'null'})\n`);
    }
  } catch (e) {
    process.stderr.write(`[TIKTOK-PROBE] error: ${e.message}\n`);
  }
