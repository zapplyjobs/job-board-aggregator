...
async function fetchAllTiktokJobs() {
  ...
  while (allJobs.length < MAX_JOBS) {
    ...
    // AGG-TIKTOK-DATE-1 DEBUG: log first post's keys to find the date field name
    if (offset === 0 && posts.length > 0) {
      console.log(`  TikTok API post keys: ${Object.keys(posts[0]).join(', ')}`);
      const p0 = posts[0];
      for (const k of Object.keys(p0)) {
        if (/time|date|create|update|publish/i.test(k)) console.log(`    ${k}=${p0[k]}`);
      }
    }
