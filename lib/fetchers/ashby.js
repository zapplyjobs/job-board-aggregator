...
async function fetchAshbyJobsGraphQL(companySlug, companyName) {
    ...
    const teamsById = new Map((result.data.data.jobBoard.teams || []).map(team => [team.id, team]));
    const fetchedAt = new Date().toISOString();
    const jobs = result.data.data.jobBoard.jobPostings || [];
    return jobs.map(job => normalizeAshbyGraphQLJob(job, companySlug, companyName, teamsById, fetchedAt));
}
