 ...
 */
async function fetchAshbyJobs(companySlug, companyName, options = {}) {
    const url = `${BASE_URL}/${companySlug}?includeCompensation=true`;
    const result = await getJson(url);

    if (result.timeout) {
        console.log(`   ⚠️ Ashby timeout: ${companySlug} after ${REQUEST_TIMEOUT_MS / 1000}s`);
        return [];
    }

    if (result.status === 404 && companySlug === 'whatnot') {
        console.log(`   ⚠️ Ashby legacy API 404: ${companySlug} — falling back to hosted board GraphQL`);
    ...
    if (jobs.length === 0 && options.fallbackGraphQLOnEmpty) {
        console.log(`   ⚠️ Ashby primary API returned 0 jobs: ${companySlug} — falling back to hosted board GraphQL`);
        return fetchAshbyJobsGraphQL(companySlug, companyName);
    }
...
async function fetchAllAshbyJobs(companies, options = {}) {
    ...
    for (let i = 0; i < companies.length; i += concurrency) {
        ...
        const results = await Promise.all(batch.map(async company => {
            ...
            try {
                const jobs = await fetchAshbyJobs(slug, name, typeof company === 'object' ? company : {});
