...
const { normalizeSalary } = require('./salary');
const { CANADA_PROVINCE_NAME_TO_ABBR } = require('./country-resolver');
...
function parseAshbyLocation(locationStr) {
  if (!locationStr) return { job_city: null, job_state: null };
  // Try "City, Province" or "City, Province, Country" patterns
  const parts = locationStr.split(",").map(s => s.trim());
  if (parts.length >= 2) {
    // Check if parts[1] is a 2-letter province code
    if (/^[A-Z]{2}$/.test(parts[1])) return { job_city: parts[0], job_state: parts[1] };
    // Check if parts[1] is a full province name
    const provAbbr = CANADA_PROVINCE_NAME_TO_ABBR[parts[1].toLowerCase()];
    if (provAbbr) return { job_city: parts[0], job_state: provAbbr };
    // AGG-LOC: Check if parts[1] is a full US state name (e.g. "San Francisco, California")
    const stateAbbr = STATE_NAME_TO_ABBR[parts[1].toLowerCase()];
    if (stateAbbr) return { job_city: parts[0], job_state: stateAbbr };
  }
  return { job_city: parts[0] || null, job_state: null };
}
...
function normalizeAshbyJob(job, companySlug, companyName) {
    ...
    return {
        ...
        salary: compensation,
        ...normalizeSalary(job.compensation?.compensationTierSummary),
...
function normalizeAshbyGraphQLJob(job, companySlug, companyName, teamsById, fetchedAt = new Date().toISOString()) {
    ...
    return {
        ...
        salary: compensation,
        ...normalizeSalary(job.compensationTierSummary),
        posted_at: job.publishedDate ? new Date(`${job.publishedDate}T00:00:00.000Z`).toISOString() : fetchedAt,
        first_published: job.publishedDate ? new Date(`${job.publishedDate}T00:00:00.000Z`).toISOString() : fetchedAt,
        fetched_at: fetchedAt,
        description: null,
        _raw: {
