...
const PAGE_SIZE = 100;
const DELAY_MS = 200;
const FILTER_PARAMS = () => `location=${encodeURIComponent(resolveCountry("amd", "US"))}&limit=100`;
...
async function fetchAmdPage(url, label) {
  const primary = await getJson(url, { headers: HEADERS });
  if (primary?.status === 200 && primary.data) return primary;
  if (primary?.status === 403 || primary === null) {
    console.log(`  ${label}: retrying with browser headers (${primary?.status || 'null'})`);
    await delay(1000);
    const retry = await getJson(url, { headers: BROWSER_HEADERS });
...
async function fetchAllAmdJobs() {
  ...
  let firstResult = await fetchAmdPage(firstUrl, 'Page 1 (filtered)');

  if (!firstResult || firstResult.status !== 200 || !firstResult.data) {
    console.log('  ⚠️  Filtered page 1 unavailable, falling back to unfiltered fetch');
    usedFilter = false;
    const fallbackUrl = `${BASE_URL}${API_PATH}?page=${page}`;
    firstResult = await fetchAmdPage(fallbackUrl, 'Page 1 (unfiltered)');
    if (!firstResult || firstResult.status !== 200 || !firstResult.data) {
      console.log(`  Page 1 (unfiltered): status=${firstResult?.status || 'null'}, stopping`);
      return [];
    }
  }
  ...
  if (usedFilter && totalCount === 0) {
    console.log('  ⚠️  Location filter returned 0, falling back to unfiltered fetch');
    usedFilter = false;
    const fallbackUrl = `${BASE_URL}${API_PATH}?page=${page}`;
    const fallbackResult = await fetchAmdPage(fallbackUrl, 'Page 1 (unfiltered)');
    ...
    while (page <= totalPages) {
      ...
      const result = await fetchAmdPage(url, `Page ${page} (unfiltered)`);
    ...
    while (page <= totalPages) {
      ...
      const result = await fetchAmdPage(url, `Page ${page}`);
