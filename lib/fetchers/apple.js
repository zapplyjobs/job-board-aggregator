...
async function fetchDetailPages(normalizedJobs, cachedIds, dataDir) {
  const MAX_DETAIL_PER_RUN = 50; // AGG-SLOWLANE-SPEED-1: reduced from 100 — 50 sequential pages × 2s = 100s + listing 85s = 185s total, comfortable margin under 300s timeout
...
async function fetchAllAppleJobs({ previousJobCount = 0, previousJobIds, cachedDescriptionIds, dataDir, skipDetails = false } = {}) {
  ...
  for (let caPage = 1; caPage <= 5; caPage++) {
    const caUrl = `${BASE_URL}${SEARCH_PATH}?sort=newest&location=canada-CANC&page=${caPage}`;
