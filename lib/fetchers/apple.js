 ...
 * Skips jobs whose IDs are already in the sidecar cache (AGG-FETCH-10).
 *
 ...
 */
async function fetchDetailPages(normalizedJobs, cachedIds) {
  const toFetch = cachedIds
    ? normalizedJobs.filter(j => !cachedIds.has(j.id))
    : normalizedJobs;
  const cachedCount = normalizedJobs.length - toFetch.length;

  if (cachedCount > 0) {
    console.log(`  Cache: ${cachedCount}/${normalizedJobs.length} already in sidecar, ${toFetch.length} need fetching`);
  }
...
async function fetchAllAppleJobs({ previousJobCount = 0, previousJobIds, cachedDescriptionIds } = {}) {
  ...
  // Fetch detail pages for qualifications (AGG-FETCH-9, cached with AGG-FETCH-10)
  await fetchDetailPages(allJobs, cachedDescriptionIds);
