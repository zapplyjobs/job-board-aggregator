...
const CONCURRENCY = 10;        // AGG-DESC-SPEED-1: parallel fetches to DIFFERENT workday tenants (no single-server hammering)
...
async function fetchWorkdayDescriptions(workdayJobs, dataDir) {
  ...
  // AGG-DESC-PRUNE-1: Prune stale entries when cache exceeds 110% of pool size.
  // Bounds the cache — descriptions for retired/dropped jobs are evicted periodically.
  const poolIds = new Set(workdayJobs.map(j => j.id));
  if (existing.size > poolIds.size * 1.1) {
    pruneDescriptions(filePath, existing, poolIds);
  }
