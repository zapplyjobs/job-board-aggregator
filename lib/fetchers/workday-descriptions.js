...
const MAX_PER_RUN = 2500;      // AGG-WORKDAY-RETRIEVABLE-1 (2026-07-20): raised from 1000 — wd-desc-queue.jsonl backlog at 20K+ jobs (969 tech-US t0 alarm); desc-backfill was clearing only 6K/hour vs arrival rate. Workday on Cloudflare (not Akamai per AGG-WD-KEEPALIVE-1), concurrency 20 produces 0 429s, has headroom. Previous: 200→1000 (AGG-DESC-SPEED-1).
const CONCURRENCY = 20;        // AGG-RUNTIME-ALERT-1: raised from 10 — requests hit different tenant subdomains, safe to parallelize more
...
async function fetchWorkdayDescriptions(workdayJobs, dataDir, options = {}) {
  ...
  if (options.skipFetch) {
    console.log(`📄 Workday descriptions: skipFetch=true — seeded cache only (${existing.size} entries), no new fetch (AGG-RUNTIME-ALERT-1)`);
    return existing;
  }
  ...
  // The cache is naturally bounded by the workday pool size (~35K entries). Stale entries
  // (retired jobs) are cleaned by Step 9d's broader pool check on the main pipeline side.
  // pruneDescriptions is still exported for external/test use but no longer auto-invoked.
...
module.exports = { fetchWorkdayDescriptions, stripHtml, loadDescriptions, appendDescriptions, pruneDescriptions };
