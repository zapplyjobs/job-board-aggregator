...
async function fetchDetailDescriptions(jobs, company) {
  const baseUrl = company.base_url.replace(/\/$/, '');
  const detailPath = '/hcmRestApi/resources/latest/recruitingCEJobRequisitionDetails';
  let fetched = 0;
  let failed = 0;

  for (const job of jobs) {
    if (!job.source_id) continue;
    const url = `${baseUrl}${detailPath}?onlyData=true&finder=ById;Id=${job.source_id}`;
    try {
      const result = await getJson(url);
      if (result?.status === 200 && result.data?.items?.length > 0) {
        const detail = result.data.items[0];
        const descParts = [];
        if (detail.ExternalDescriptionStr) descParts.push(detail.ExternalDescriptionStr.trim());
        if (detail.ExternalResponsibilitiesStr) descParts.push('Responsibilities:\n' + detail.ExternalResponsibilitiesStr.trim());
        if (detail.ExternalQualificationsStr) descParts.push('Qualifications:\n' + detail.ExternalQualificationsStr.trim());
        if (descParts.length) {
    ...
    if (fetched + failed < jobs.length) await delay(DETAIL_DELAY_MS);
  }

  console.log(`  ${company.name} detail fetch: ${fetched} enriched, ${failed} failed of ${jobs.length}`);
}
...
async function fetchAllOracleJobs(companies) {
  ...
  for (const company of companies) {
    try {
      ...
      await fetchDetailDescriptions(jobs, company);
      allJobs.push(...jobs);
    } catch (err) {
  ...
  console.log(`  Oracle HCM total: ${allJobs.length} jobs from ${companies.length} companies`);
