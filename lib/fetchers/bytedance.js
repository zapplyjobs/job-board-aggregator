...
async function fetchAllByteDanceJobs() {
  ...
  while (allJobs.length < MAX_JOBS) {
    ...
    for (const post of posts) {
      // AGG-SIMPLIFY-EXIT-1: filter to US only (removed city-code filter to catch all US cities)
      const country = post.city_info?.parent?.parent?.en_name || '';
      if (!/United States|USA|US/i.test(country)) continue;
