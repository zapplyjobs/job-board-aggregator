  ...
  async function downloadToFile(name, destPath) {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        // A89 OOM fix used Readable.fromWeb(), but AWS SDK v3 wraps resp.Body
        // in a ChecksumStream (extends Node Readable, not Web ReadableStream).
        // Readable.fromWeb() rejects ChecksumStream. Fix: detect stream type.
        const body = resp.Body;
        if (body && typeof body.pipe === 'function') {
          // Node.js Readable (including ChecksumStream) — pipe directly
          body.pipe(ws);
        } else if (body && typeof body.getReader === 'function') {
          // Web ReadableStream — convert via Readable.fromWeb()
          Readable.fromWeb(body).pipe(ws);
        } else {
          reject(new Error('Unexpected body type: ' + typeof body));
          return;
        }
      ...
      console.error(`R2 downloadToFile FAILED: ${name} — ${err.message}`);
      return null;
    }
  }
