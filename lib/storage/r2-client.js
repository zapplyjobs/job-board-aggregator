...
function createR2Client(options = {}) {
  ...
  /**
   * Delete objects by name (batch). Used to prune SUPERSEDED sidecar files from R2
   * (AGG-R2-SINGLEFILE-1). Caller MUST pass only keys the producer explicitly marked
   * stale — never blanket-delete; old ATS descriptions are D2-protected.
   */
  async function deleteObjects(names) {
    const keys = (Array.isArray(names) ? names : [names]).map(n => key(n)).filter(Boolean);
    if (keys.length === 0) return 0;
    let deleted = 0;
    for (let i = 0; i < keys.length; i += 1000) {
      const batch = keys.slice(i, i + 1000);
      try {
        const resp = await sendWithRetry(() => client.send(new DeleteObjectsCommand({
          Bucket: bucket,
          Delete: { Objects: batch.map(k => ({ Key: k })), Quiet: true },
        })), `delete ${batch.length} objects`);
        deleted += resp && resp.Deleted ? resp.Deleted.length : batch.length;
      } catch (err) {
        logFailure('delete', keys.join(','), err);
      }
    }
    console.log(`R2 delete OK: ${deleted} object(s) removed`);
    return deleted;
  }
