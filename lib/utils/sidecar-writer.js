...
function writeSidecars(sortedJobs, dataDir, options = {}) {
  ...
  for (const job of sortedJobs) {
    ...
    if (job.description) {
      // AGG-ORACLE-DEPT: persist captured departments alongside the description so they
      // survive the description-cache skip on later runs (Oracle Category is detail-only;
      // once captured it must not be lost when the job is no longer detail-fetched).
      const entry = { id: job.id, description_text: job.description };
      if (Array.isArray(job.departments) && job.departments.length > 0) {
        entry.departments = job.departments;
      }
      bySource[src].push(entry);
    }
  }
  // AGG-MICROSOFT-RETRIEVABLE-1 DEBUG: trace why microsoft descriptions from supplemental don't reach sidecar
  const _debugSources = ['microsoft', 'bytedance', 'oracle'];
  for (const _ds of _debugSources) {
    const _all = sortedJobs.filter(j => j.source === _ds);
    const _withDesc = _all.filter(j => j.description);
    const _inBySource = (bySource[_ds] || []).length;
    if (_all.length > 0) console.log(`🔍 DEBUG sidecar-grouping ${_ds}: ${_all.length} in sortedJobs, ${_withDesc.length} with desc, ${_inBySource} grouped for sidecar write`);
  }

  // Accumulate prior entries across runs (ENR-2 fix; AGG-ORACLE-DEPT also carries departments)
  for (const src of Object.keys(bySource)) {
    const priorMap = new Map();   // id -> { description_text, departments }
    ...
    for (const fname of priorFiles) {
      ...
      for (const line of lines) {
        try {
          ...
          if (id && description_text) priorMap.set(id, { description_text, departments: Array.isArray(departments) ? departments : null });
        } catch (_) {}
      }
    }
    ...
    for (const entry of bySource[src]) {
      ...
      const prior = priorMap.get(entry.id);
      merged.set(entry.id, {
        description_text: chooseDescription(src, prior && prior.description_text, entry.description_text),
        // Departments are immutable per job: prefer the current capture, else keep the prior one.
        departments: (entry.departments && entry.departments.length) ? entry.departments : (prior && prior.departments) || null,
      });
    }
    ...
    bySource[src] = Array.from(merged, ([id, v]) => {
      const e = { id, description_text: v.description_text };
      if (v.departments && v.departments.length) e.departments = v.departments;
      return e;
    });
  }
